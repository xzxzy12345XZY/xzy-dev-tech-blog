...
`,S=(t,{type:M,slugify:d},$,m=!1)=>Promise.all(M.map(async({key:a,sorter:f=()=>-1,filter:k=()=>!0,path:l="/:key/",layout:_="Layout",frontmatter:E=()=>({})},v)=>{if(!U(a)||!a.length)return s.error(`Invalid ${u.magenta("key")} option ${u.cyan(a)} in ${u.cyan(`type[${v}]`)}`),null;const b={},w=[];t.env.isDebug&&s.info(`Generating ${u.cyan(a)} type.
`);for(const g in $){const e=$[g].filter(k).sort(f).map(({key:c})=>c);if(l){const c=`${g}${D(d(l.replace(/:key/g,a)))}`,o=await P(t,{path:encodeURI(c),frontmatter:{...E(g),blog:{type:"type",key:a},layout:_}}),p=t.pages.findIndex(({path:y})=>y===c);p===-1?t.pages.push(o):t.pages[p].key!==o.key&&(t.pages.splice(p,1,o),m&&s.warn(`Overriding existed path ${u.cyan(c)}`)),w.push(o.key),b[g]={path:o.path,keys:e},t.env.isDebug&&s.info(`Route ${g} in ${a} type: path: ${o.path}; items: ${e.length}
`)}else b[g]={path:"",keys:e},t.env.isDebug&&s.info(`Route ${g} in ${a} type: items: ${e.length}
`)}return{key:a,map:b,pageKeys:w}})).then(async a=>{const f={},k=[];return a.filter(l=>l!==null).forEach(({key:l,map:_,pageKeys:E})=>{f[l]=_,k.push(...E)}),await t.writeTemp("blog/type.js",`export const typeMap = ${JSON.stringify(f)};
${t.env.isDev?Y:""}
`),t.env.isDebug&&s.info("All types generated."),k}),Z=(t,M=!0)=>d=>{M&&X(t),z(d,C,"2.0.0-beta.67");const{getInfo:$=()=>({}),filter:m=e=>!!e.filePathRelative&&!e.frontmatter.home,metaScope:a="_blog",excerpt:f=!0,excerptSeparator:k="<!-- more -->",excerptLength:l=300,excerptFilter:_=m,isCustomElement:E=()=>!1,category:v=[],type:b=[],slugify:w=e=>e.replace(/[ _]/g,"-").replace(/[:?*|\\/<>]/g,"").toLowerCase()}=t;let g=[];return d.env.isDebug&&s.info("Options:",t),{name:C,define:()=>({BLOG_META_SCOPE:a}),extendsPage:e=>{f&&_(e)&&(e.data.excerpt=W(d,e,{isCustomElement:E,excerptSeparator:k,excerptLength:l}))},extendsBundlerOptions:(e,c)=>{q(e,c,"vuepress-shared")},onInitialized:e=>{const c=N(e,m);return e.pages.filter(m).forEach(o=>{o.routeMeta={...a===""?$(o):{[a]:$(o)},...o.routeMeta}}),Promise.all([O(e,{category:v,slugify:w},c,!0).then(o=>{g.push(...o)}),S(e,{type:b,slugify:w},c,!0).then(o=>{g.push(...o)})]).then(()=>{e.env.isDebug&&s.info("temp file generated")})},onWatched:(e,c)=>{if("hotReload"in t?t.hotReload:e.env.isDebug){const o=j("pages/**/*.js",{cwd:e.dir.temp(),ignoreInitial:!0}),p=()=>{const y=[],x=N(e,m);return Promise.all([O(e,{category:v,slugify:w},x).then(r=>{y.push(...r)}),S(e,{type:b,slugify:w},x).then(r=>{y.push(...r)})]).then(async()=>{const r=g.filter(n=>!y.includes(n)),i=y.filter(n=>!g.includes(n));i.length&&(e.env.isDebug&&s.info(`New pages detected: ${i.toString()}`),await Promise.all(i.map(async n=>{await B(e,e.pages.find(({key:h})=>h===n)),await V(e,e.pages.find(({key:h})=>h===n))}))),r.length&&(e.env.isDebug&&s.info(`Removing following pages: ${r.toString()}`),r.forEach(n=>{e.pages.splice(e.pages.findIndex(({key:h})=>h===n),1)})),(r.length||i.length)&&(await A(e),await G(e),await K(e)),g=y,e.env.isDebug&&s.info("temp file updated")})};o.on("add",()=>{p()}),o.on("change",()=>{p()}),o.on("unlink",()=>{p()}),c.push(o)}}}};export{Z as blogPlugin};
